...
- Your spec is at `locard/specs/XXXX-spec-name.md`
- Your plan is at `locard/plans/XXXX-spec-name.md`
...
Use these commands to communicate with the Architect:

```bash
# Create a PR and notify the Architect (recommended workflow)
locard pr

# Create PR with custom title
locard pr --title "Add user authentication"

# Create as draft PR
locard pr --draft

# Send a direct message to the Architect
af send architect "Question about the API design"

# Check your current status
af status
```

**When to notify the Architect:**
- **PR Ready**: Use `locard pr` - creates PR and notifies automatically
- **Blocked**: Use `af send architect "description of blocker"`
- **Questions**: Add `<!-- REVIEW(@architect): question -->` comments in code

**Note**: Notifications are deduplicated (5-minute window) to avoid spam.

## Example Builder Session

```
1. Spawned for spec 0003-user-auth
2. Read spec at locard/specs/0003-user-auth.md
...
If a test file exists at `locard/tests/NNNN-*` (matching your spec number):

1. **Read the tests first** - They encode the spec's requirements as executable code
2. **Run tests early** - They should all fail initially (TDD red phase)
3. **Implement to pass tests** - Each passing test = progress toward completion
4. **DO NOT modify acceptance tests** - They are the spec's source of truth
5. **If a test seems wrong** - Flag it in your PR description for Architect review

Your PR will be rejected if acceptance tests are modified without Architect approval.

### TDD Workflow

```
1. Read spec + plan + tests
2. Run tests (all fail - red phase)
3. Implement minimal code to pass first test
4. Run tests (one passes)
5. Iterate until all acceptance tests pass
6. Write additional unit tests in Defend phase
7. Create PR
```
...
- **Don't modify acceptance tests** - Without Architect approval (TDD compliance)
