...
- **AC24**: Integration test: stub CDX returns clean empty array on **first** observation — assert org marked `status='transient'` with `notes='wayback_no_coverage'`, attempts=1, no PDFs downloaded, `wayback_empty` counter incremented. Second consecutive empty (existing row already `status='transient'` AND `notes='wayback_no_coverage'`) → assert promoted to `status='permanent_skip'`, attempts=2. (Two-strikes empty rule per AC15.6.)
...
- **AC24.4**: Integration test: stub CDX returns 200 with `[[header],]` (header-only, zero data rows) — assert classified as `EMPTY`, org marked `status='transient'` with `notes='wayback_no_coverage'` (first observation; promotion to `permanent_skip` occurs on the second consecutive empty per the two-strikes rule).
