...
- The `consult` CLI must be available (installed with `npm install -g @locard/cli`)
- At least one consultation backend: `claude`, `gemini-cli`, or `codex`
- Check with: `locard doctor` or `consult --help`
...
7. **Red Team Security Review (MANDATORY)**
   - Run AFTER expert consultations, BEFORE human approval
   - Command: `consult --model gemini --type red-team-spec spec XXXX`
   - Focus: Design flaws, threat model gaps, attack surface analysis
   - Address ALL security findings before proceeding (CRITICAL, HIGH, MEDIUM, LOW)
   - Document findings and mitigations in "Security Review" section of spec
   - **BLOCKING**: Cannot proceed with ANY findings unresolved

**⚠️ BLOCKING**: Cannot proceed without BOTH consultations AND red team review (unless explicitly disabled)

**Output**: Single specification document in `locard/specs/####-descriptive-name.md`
...
**Template**: `locard/protocols/spider/templates/spec.md`
...
4. **Red Team Security Review (MANDATORY)**
   - Run AFTER expert consultations, BEFORE human approval
   - Command: `consult --model gemini --type red-team-plan plan XXXX`
   - Focus: Implementation approach vulnerabilities, crypto choices, dependency risks
   - Address ALL security findings before proceeding (CRITICAL, HIGH, MEDIUM, LOW)
   - Document findings and mitigations in "Security Review" section of plan
   - **BLOCKING**: Cannot proceed with ANY findings unresolved

**⚠️ BLOCKING**: Cannot proceed without BOTH consultations AND red team review (unless explicitly disabled)

**Output**: Single plan document in `locard/plans/####-descriptive-name.md`
...
- Version control captures evolution through commits
**Template**: `locard/protocols/spider/templates/plan.md`
...
- Run AFTER expert consultation, BEFORE moving to Defend phase
- Command: `consult --model gemini --type red-team-impl pr XX` (or specify files)
- Focus: OWASP Top 10, exploitable vulnerabilities, injection flaws, auth bypasses
- Address ALL security findings before proceeding (CRITICAL, HIGH, MEDIUM, LOW) to tests
- **BLOCKING**: Cannot proceed with ANY findings unresolved
...
3. **Update Architecture Documentation**
   - Update `locard/resources/arch.md` with new modules, utilities, or architectural changes
...
Templates for each phase are available in `locard/protocols/spider/templates/`:
