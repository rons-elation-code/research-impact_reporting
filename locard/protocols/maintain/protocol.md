...
- `locard/resources/arch.md` - Architecture documentation (how the system works)
- `locard/resources/lessons-learned.md` - Extracted wisdom from reviews
...
1. Check `locard/maintain/` for last maintenance run
...
Each maintenance run creates a numbered file in `locard/maintain/`:
...
**Template**: `locard/protocols/maintain/templates/maintenance-run.md`
...
| Task | Parallelizable | Human Review? | Description |
|------|----------------|---------------|-------------|
| Run locard update | No | Yes | Update Locard framework to latest version |
...
Scan the actual codebase and update `locard/resources/arch.md`:
...
4. Review new/modified specs: `git diff <base-commit>..HEAD --name-only -- locard/specs/`
5. Review new/modified plans: `git diff <base-commit>..HEAD --name-only -- locard/plans/`
...
Extract actionable wisdom from review documents into `locard/resources/lessons-learned.md`:

**Discovery phase**:
1. Find new/modified reviews: `git diff <base-commit>..HEAD --name-only -- locard/reviews/`
2. Read each new/modified review file

**Extract from reviews**:
1. Read all files in `locard/reviews/`
...
**Files to review**:
- `locard/resources/arch.md` - remove references to deleted code/modules
- `locard/resources/lessons-learned.md` - remove outdated lessons
...
- Detailed command references → `locard/resources/commands/`
- Protocol details → `locard/protocols/*/protocol.md`
- Tool configuration → `locard/resources/`
...
- **When removing duplicated content**: Replace with a pointer to the canonical location
  ...
  See `locard/resources/commands/consult.md` for full documentation.
  ```
...
Update the Locard framework to the latest version:
...
### For untracked files
Move to `locard/maintain/.trash/YYYY-MM-DD/` before deleting. Retained for 30 days.
